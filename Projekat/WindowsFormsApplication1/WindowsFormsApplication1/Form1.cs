...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        public static Figura trenutnaFigura = null;
        public static Figura sledecaFigura = null;
        //
        ...
        public static int robotP { get; set; }
        public static int igracP { get; set; }

        public static Boolean nasaoJe = true;
        public static Boolean prviKrug = true;

        public static System.Timers.Timer countdownTimer;

        public static int vreme { get; set; }

        public static Boolean krajIgre = false;
        public static object lockObject = new object();
        public static object lockObject2 = new object();

        public static int brojPotezaRobot { get; set; }
        public static int brojPotezaIgrac { get; set; }
        ...
        public Form1()
        {
            ...
            ZELENA.Tag = 6;
            KOMBO.Tag = 7;
            Console.WriteLine(ZELENA.Tag + " OVO JE TAG!!!!!");
            napraviRezultantnePanele();
            napraviRadnePanele();
            napraviPaneleZaSledecuFiguru();

            napraviPocetnuFiguru();
            napraviSledecuFiguru();

            napraviRezultantnePaneleIgrac(Igrac.rezultantniPaneli);
            napraviRadnePaneleIgrac(Igrac.radniPaneli);
            napraviPaneleZaSledecuFiguruIgrac(Igrac.paneliOdSledeceFigure);

            Igrac i = new Igrac();
            Igrac.napraviPocetnuFiguru();
            Igrac.napraviSledecuFiguru();
        ...
        public static void beginCountdown()
        {
            ...
            countdownTimer.Elapsed += odbrojavaj;
            countdownTimer.AutoReset = true;
            countdownTimer.Enabled = true;
        }

        public static void odbrojavaj(Object source, ElapsedEventArgs e)
        {
            if (vreme == 0)
            {
                krajIgre = true;
                ...
                aTimer.Stop();
                Igrac.bTimer.Stop();
                Robot.GLAVNI_TIMER.Stop();
                Instance.ZavrsiIgruIProglasiPobednika();
            }
            else
            {
                vreme--;
                Instance.refreshujVreme();
            }

        }

        public void refreshujVreme()
        {
            this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label6.Text = "- " + vreme.ToString() + " -"; });
        }
        public void ZavrsiIgruIProglasiPobednika()
        {
            if (robotP > igracP)
            {
                this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label7.Text = "Pobedio je ROBOT!"; });
            }
            else if (robotP < igracP)
            {
                this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label7.Text = "Pobedio je Čovek!"; });
            }
            else
            {
                this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label7.Text = "NEREŠENO JE!"; });
            }
        }

        public static void napraviTrenutnuFiguru(Figura f)
        {
        ...
        public static Figura napraviFiguru()
        {
            ...
            Figura f = new Figura(figura[1], figura[2], figura[3], figura[4]);
            return f;
        }

        public void napraviRezultantnePanele()
        {
            //Random rnd = new Random();
            for (int i = 0; i < 16; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    ...
                    polje.Size = new Size(32, 32);
                    Point p = new Point(32 * i, 32 * j);
                    Console.WriteLine("X:" + p.X + ", Y:" + p.Y);
                    polje.Location = p;
                    polje.BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
                    panel1.Controls.Add(polje);
                    rezultantniPaneli.Add(p, polje);
                }
            }
        }
        ...
        public void napraviRadnePanele()
        {
            for (int i = 0; i < 16; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    polje.BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
                    //polje.Image = PLAVA;
                    panel2.Controls.Add(polje);
                    radniPaneli.Add(p, polje);
                }
            }
        }

        public void napraviPaneleZaSledecuFiguru()
        {
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    polje.BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
                    //polje.Image = PLAVA;
                    panel3.Controls.Add(polje);
                    paneliOdSledeceFigure.Add(p, polje);
                }
            }
        }
        ...
        public class Figura
        {
            public Image prvaSlika { get; set; }
            public Image drugaSlika { get; set; }
            public Image trecaSlika { get; set; }
            public Image cetvrtaSlika { get; set; } 

            public Figura(Image jedan, Image dva, Image tri, Image cetiri)
            {
                prvaSlika = jedan;
                drugaSlika = dva;
                trecaSlika = tri;
            ...
            public void rotirajGore()
            {
                Image temp = prvaSlika;
                prvaSlika = drugaSlika;
                drugaSlika = cetvrtaSlika;
                cetvrtaSlika = trecaSlika;
                trecaSlika = temp;
            }
            public void rotirajDole()
            {
                Image temp = prvaSlika;
                prvaSlika = trecaSlika;
                trecaSlika = cetvrtaSlika;
                cetvrtaSlika = drugaSlika;
                drugaSlika = temp;
            }
        ...
        public void Form1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (Igrac.DOZVOLJENO == true)
            {
                if (e.KeyChar == ' ')
                {
                    if (krajIgre == false)
                    {
                        if (Igrac.rezultantniPaneli[Igrac.trenutnoPrvo].Image == null || Igrac.rezultantniPaneli[Igrac.trenutnoDrugo].Image == null || Igrac.rezultantniPaneli[Igrac.trenutnoTrece].Image == null || Igrac.rezultantniPaneli[Igrac.trenutnoCetvrto].Image == null)
                        {
                            Instance.povecajPotezIgrac();
                            Igrac.spustiFiguru();
                        }
                    }
                }
        ...
        */
        ...
        {
            // Create a timer with a two second interval.
            aTimer = new System.Timers.Timer(105);
            // Hook up the Elapsed event for the timer. 
        ...
        public void povecajPotezRobot()
        {
            brojPotezaRobot++;
            this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label8.Text = brojPotezaRobot.ToString() + " <--- Broj Poteza"; });
        }

        public void povecajPotezIgrac()
        {
            brojPotezaIgrac++;
            this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label9.Text = "Broj Poteza ---> " + brojPotezaIgrac.ToString(); });
        }
        ...
        public void povecajBrojPoenaIgrac(int i)
        {
            igracP += i;
        }

        public void povecajBrojPoenaRobot(int i)
        {
            robotP += i;
        }

        public void refreshujPoene()
        {
            this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label1.Text = igracP.ToString(); });
            this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate() { label2.Text = robotP.ToString(); });
        }
        ...
        */
        ...
        public void napraviRezultantnePaneleIgrac(Dictionary<Point, PictureBox> rp)
        {
            //Random rnd = new Random();
            for (int i = 0; i < 16; i++)
            {
                for (int j = 0; j < 16; j++)
                {
                    ...
                    polje.Location = p;
                    polje.BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
                    panel6.Controls.Add(polje);
                    rp.Add(p, polje);
                }
            }
        }

        public void napraviRadnePaneleIgrac(Dictionary<Point, PictureBox> rp)
        {
            for (int i = 0; i < 16; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    polje.BorderStyle = System.Windows.Forms.BorderStyle.FixedSingle;
                    //polje.Image = PLAVA;
                    panel5.Controls.Add(polje);
                    rp.Add(p, polje);
                }
            }
        }

        public void napraviPaneleZaSledecuFiguruIgrac(Dictionary<Point, PictureBox> sf)
        {
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
